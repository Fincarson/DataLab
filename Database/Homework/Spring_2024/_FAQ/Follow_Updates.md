...
1. 將助教的 repository 的 clone 網址複製下來。例如，作業一的 clone URL 是：`https://shwu10.cs.nthu.edu.tw/courses/databases/2022-spring/db22-assignment-1.git`
...
3. 使用 `git remote add [名稱] [URL]` 將助教的 repository 加入 remote 清單中。例如將作業一的 URL 以 `tas` 的名稱放入：
    ...
    ```bash
    > git remote add tas https://shwu10.cs.nthu.edu.tw/courses/databases/2022-spring/db22-assignment-1.git
    ```

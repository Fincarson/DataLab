...
exports.redistributeOrphanTodos = onDocumentDeleted(
  {
  ...
  async (event) => {
    ...
    try {
      // Get all todo-items of deleted user
      const todosSnapshot = await db
        .collection(`apps/group-todo-list/users/${deletedUserId}/todo-items`)
        .get();

      if (todosSnapshot.empty) {
        console.log(`No todos to redistribute for user ${deletedUserId}`);
        return null;
      }
      ...
      const otherUsers = usersSnapshot.docs
        .filter(doc => doc.id !== deletedUserId)
        .map(doc => doc.id);

      if (otherUsers.length === 0) {
        console.log("No other users to redistribute todos to");
        return null;
      }

      // Round-robin redistribute todos
      let i = 0;
      const batch = db.batch();

      todosSnapshot.forEach((todoDoc) => {
        const newOwnerId = otherUsers[i % otherUsers.length];
        const newTodoRef = db
          .collection(`apps/group-todo-list/users/${newOwnerId}/todo-items`)
          .doc(todoDoc.id);
        batch.set(newTodoRef, todoDoc.data());
        batch.delete(todoDoc.ref);
        i++;
      });

      await batch.commit();
      console.log(`Redistributed todos of user ${deletedUserId} to other users.`);
      return null;
    } catch (error) {
      console.error("Error redistributing orphan todos:", error);
      return null;
    }
  }
);

...
export const customRecipeFlow = ai.defineFlow({
    ...
    async (input) => {
            ...
            async () => {
                try{
                    const docs = await ai.retrieve({
                        ...
                        query: input.ingredients, // updated
                        options: {
                            limit: 1,
                        },
                    });
        ...
        const response = await recipeGenerator(
            ...
            ingredients: input.ingredients // updated
         });

        // original recipe image
        const imageResponse1 = await imageGenerator(
            {
        ...
        // custom recipe image
        const imageResponse2 = await imageGenerator(
            {
                title: response.output.title,
                ingredients: response.output.ingredients,
                directions: response.output.directions
            }
        );
        ...
        return { 
            recipe: customRecipe,
            customRecipeImage: imageResponse2,
            originRecipeImage: imageResponse1
        };
